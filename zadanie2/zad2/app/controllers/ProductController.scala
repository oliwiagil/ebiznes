...
class ProductController @Inject()(val controllerComponents: ControllerComponents) extends BaseController {
  ...
  productList+=Product(1,"first")
  productList+=Product(2,"second")
  productList+=Product(3,"third")
  ...
  def updateProduct(id: Long): Action[AnyContent] = Action { implicit request =>
    ...
    if (index != -1) {
     ...
     proposedProduct match {
       case Some(pr) =>
         val updatedProduct = Product(id, pr.name)
  ...
  def addProduct: Action[AnyContent] = Action { implicit request =>
    ...
    product match {
      case Some(newProduct) =>
        ...
        val addedProduct = Product(nextId, newProduct.name)
